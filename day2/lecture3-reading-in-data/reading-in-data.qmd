...
# On Reading Data into R {.white-title background-image="images/bg-collatz-2.png" background-size="cover"}
...
- **So far, we've learned a bit about:**
  -   [Data documentation]{.blue-bold} and its importance for reproducible research
...
-   **Learn about [common storage methods]{.blue-bold} you may encounter as researcher**
    -   Flat data files
...
-  **Learn [different approaches for reading data]{.blue-bold} into R**
    -   Base R functions
...
-   **Understand the [potential challenges]{.blue-bold} one may encounter when reading data into R**
    -   File types
    -   Data formatting
    -   Data types
...
```{r, echo=T}
# Load readr package (not necessary if you've installed the tidyverse)
library(readr)
...
## Common challenges and tips {.smaller}
-   File types 
-   Data formatting
-   Data types
...
[What additional information do you need from your colleague before you provide support?]{.blue-bold}
...
-   The data structure issue can be resolved fairly easily
    -   Use the `skip` argument skip the non-rectangular data at the top of the file
    -   After rows are skipped, drop the first row of data containing units using `[-1,]`

```{r}
# The data formatting issue can be 
data <- read_xlsx("data/exercise.xlsx", skip = 3, col_types = "text", )[-1,]
...
[... But there's also an issue with the time variable, which should be formatted as minutes:seconds. Fixing this might be tricky, but let's give it a shot.]{.blue-bold}


## Case study 2  (cont.) {.smaller}
-   Add code to correct data type issue (incorrect time variable: should be M:S, source data does not have leading 0)

## A note on exporting data {.smaller}

- Add table 

- package1 | read_* | write_*
  - etc.
...
-   Importing data is often straightforward, but there are potential challenges:
  -   Idiosyncrasies data storage methods warrant specific considerations. For example:
