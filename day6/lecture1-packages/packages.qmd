...
share, or a dataset that you want to make not just widely accessible and popular, then a package will reduce the friction in doing
so.
...
first, make sure you're in the directory where you want to create your package. then
you might run `usethis::create_package(path = "myNewPackage")`. That's going to create a new project for you with the boilerplate for a package:
...
you should put _functions_ and _data documentation_ inside scripts in the 
`R/` folder.
...
just like R Markdown you can render a vignette using Cmd+Shift+K or Ctrl+Shift+K
or click Knit in your RStudio window. 
...
the definitive reference is the R Packages book by 
Hadley Wickham and Jenny Bryan https://r-pkgs.org
